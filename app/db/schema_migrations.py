...
def ensure_custom_test_restructure_tables() -> None:
    """Expand-only tables for normalized custom test intent, projections, and snapshots."""
    id_column = (
    ...
    with engine.begin() as conn:
        conn.exec_driver_sql(
            f"""
            CREATE TABLE IF NOT EXISTS admin_custom_test_source (
                id {id_column},
                custom_test_id INTEGER NOT NULL REFERENCES child_test(id),
                source_test_id VARCHAR(50) NOT NULL,
                source_order INTEGER NOT NULL DEFAULT 0,
                is_enabled BOOLEAN NOT NULL DEFAULT TRUE,
                created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
                CONSTRAINT uq_custom_test_source UNIQUE (custom_test_id, source_test_id)
            )
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_source_custom_test_id
            ON admin_custom_test_source (custom_test_id)
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_source_source_test_id
            ON admin_custom_test_source (source_test_id)
            """
        )

        conn.exec_driver_sql(
            f"""
            CREATE TABLE IF NOT EXISTS admin_custom_test_scale_selection (
                id {id_column},
                custom_test_source_id INTEGER NOT NULL REFERENCES admin_custom_test_source(id),
                scale_code VARCHAR(80) NOT NULL,
                selected_order INTEGER NOT NULL DEFAULT 0,
                created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
                CONSTRAINT uq_custom_test_scale_selection UNIQUE (custom_test_source_id, scale_code)
            )
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_scale_selection_source_id
            ON admin_custom_test_scale_selection (custom_test_source_id)
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_scale_selection_scale_code
            ON admin_custom_test_scale_selection (scale_code)
            """
        )

        conn.exec_driver_sql(
            f"""
            CREATE TABLE IF NOT EXISTS admin_custom_test_variant_projection (
                id {id_column},
                custom_test_source_id INTEGER NOT NULL REFERENCES admin_custom_test_source(id),
                condition_hash VARCHAR(128) NOT NULL,
                eligibility_condition_json TEXT NOT NULL,
                generated_from_hash VARCHAR(128) NOT NULL,
                is_current BOOLEAN NOT NULL DEFAULT TRUE,
                status VARCHAR(40) NOT NULL DEFAULT 'current',
                generated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
                CONSTRAINT uq_custom_test_variant_projection_hash
                    UNIQUE (custom_test_source_id, condition_hash, generated_from_hash)
            )
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_variant_projection_source_id
            ON admin_custom_test_variant_projection (custom_test_source_id)
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_variant_projection_condition_hash
            ON admin_custom_test_variant_projection (condition_hash)
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_variant_projection_generated_hash
            ON admin_custom_test_variant_projection (generated_from_hash)
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_variant_projection_is_current
            ON admin_custom_test_variant_projection (is_current)
            """
        )

        conn.exec_driver_sql(
            f"""
            CREATE TABLE IF NOT EXISTS admin_custom_test_source_dependency (
                id {id_column},
                custom_test_source_id INTEGER NOT NULL REFERENCES admin_custom_test_source(id),
                dependency_type VARCHAR(40) NOT NULL,
                dependency_id VARCHAR(120) NOT NULL,
                dependency_hash VARCHAR(128) NOT NULL,
                observed_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
                CONSTRAINT uq_custom_test_source_dependency
                    UNIQUE (custom_test_source_id, dependency_type, dependency_id)
            )
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_source_dependency_source_id
            ON admin_custom_test_source_dependency (custom_test_source_id)
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_source_dependency_type
            ON admin_custom_test_source_dependency (dependency_type)
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_source_dependency_id
            ON admin_custom_test_source_dependency (dependency_id)
            """
        )

        conn.exec_driver_sql(
            f"""
            CREATE TABLE IF NOT EXISTS admin_custom_test_variant_scale_projection (
                id {id_column},
                variant_projection_id INTEGER NOT NULL REFERENCES admin_custom_test_variant_projection(id),
                scale_code VARCHAR(80) NOT NULL,
                availability_status VARCHAR(40) NOT NULL DEFAULT 'selected',
                created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
                CONSTRAINT uq_custom_test_variant_scale_projection
                    UNIQUE (variant_projection_id, scale_code)
            )
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_variant_scale_projection_variant_id
            ON admin_custom_test_variant_scale_projection (variant_projection_id)
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_variant_scale_projection_scale_code
            ON admin_custom_test_variant_scale_projection (scale_code)
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_variant_scale_projection_status
            ON admin_custom_test_variant_scale_projection (availability_status)
            """
        )

        conn.exec_driver_sql(
            f"""
            CREATE TABLE IF NOT EXISTS admin_custom_test_session (
                id {id_column},
                custom_test_id INTEGER NOT NULL REFERENCES child_test(id),
                session_index INTEGER NOT NULL DEFAULT 0,
                title VARCHAR(120) NOT NULL DEFAULT '',
                description TEXT NOT NULL DEFAULT '',
                guide_items_json TEXT NOT NULL DEFAULT '[]',
                created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
                CONSTRAINT uq_custom_test_session_index UNIQUE (custom_test_id, session_index)
            )
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_session_custom_test_id
            ON admin_custom_test_session (custom_test_id)
            """
        )

        conn.exec_driver_sql(
            f"""
            CREATE TABLE IF NOT EXISTS admin_custom_test_session_source (
                id {id_column},
                session_id INTEGER NOT NULL REFERENCES admin_custom_test_session(id),
                custom_test_source_id INTEGER NOT NULL REFERENCES admin_custom_test_source(id),
                display_order INTEGER NOT NULL DEFAULT 0,
                CONSTRAINT uq_custom_test_session_source UNIQUE (session_id, custom_test_source_id)
            )
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_session_source_session_id
            ON admin_custom_test_session_source (session_id)
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_session_source_source_id
            ON admin_custom_test_session_source (custom_test_source_id)
            """
        )

        conn.exec_driver_sql(
            f"""
            CREATE TABLE IF NOT EXISTS admin_custom_test_profile_field (
                id {id_column},
                custom_test_id INTEGER NOT NULL REFERENCES child_test(id),
                field_key VARCHAR(80) NOT NULL,
                label VARCHAR(120) NOT NULL DEFAULT '',
                input_type VARCHAR(40) NOT NULL DEFAULT 'text',
                required BOOLEAN NOT NULL DEFAULT FALSE,
                options_json TEXT NOT NULL DEFAULT '[]',
                display_order INTEGER NOT NULL DEFAULT 0,
                created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
                CONSTRAINT uq_custom_test_profile_field UNIQUE (custom_test_id, field_key)
            )
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_profile_field_custom_test_id
            ON admin_custom_test_profile_field (custom_test_id)
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_admin_custom_test_profile_field_field_key
            ON admin_custom_test_profile_field (field_key)
            """
        )

        conn.exec_driver_sql(
            f"""
            CREATE TABLE IF NOT EXISTS submission_custom_test_snapshot (
                id {id_column},
                submission_id INTEGER NOT NULL REFERENCES admin_custom_test_submission(id),
                custom_test_id INTEGER NOT NULL REFERENCES child_test(id),
                source_test_ids_snapshot_json TEXT NOT NULL DEFAULT '[]',
                variant_projection_snapshot_json TEXT NOT NULL DEFAULT '[]',
                selected_scales_snapshot_json TEXT NOT NULL DEFAULT '{{}}',
                session_configs_snapshot_json TEXT NOT NULL DEFAULT '[]',
                profile_fields_snapshot_json TEXT NOT NULL DEFAULT '[]',
                source_dependency_hash_snapshot VARCHAR(128) NOT NULL DEFAULT '',
                snapshot_source VARCHAR(40) NOT NULL DEFAULT 'submission_time',
                created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
                CONSTRAINT uq_submission_custom_test_snapshot_submission UNIQUE (submission_id)
            )
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_submission_custom_test_snapshot_submission_id
            ON submission_custom_test_snapshot (submission_id)
            """
        )
        conn.exec_driver_sql(
            """
            CREATE INDEX IF NOT EXISTS ix_submission_custom_test_snapshot_custom_test_id
            ON submission_custom_test_snapshot (custom_test_id)
            """
        )
