...
/**
 * ResearchNotice
 * 검사 실시 첫 화면(ProfileStep) 좌측 hero에 들어가는 "연구 참여 안내" 블록.
 * - 안내문 + 참여 혜택 callout + "제공되는 검사" 토글 아코디언(클릭 시 3종 한 번에 펼침)
 *
 * 데이터는 우선 상수로 둡니다. 추후 세션/관리자 설정값
 * (session.description, additional_profile_fields 등)으로 옮길 수 있습니다.
 */
...
export function ResearchNotice() {
  ...
  return (
    <div className="mt-7 w-full max-w-[560px] rounded-[18px] border border-[#e7ede9] bg-white/80 p-7 shadow-[0_1px_2px_rgba(23,94,99,0.04),0_12px_32px_-16px_rgba(23,94,99,0.22)] backdrop-blur-sm [word-break:keep-all]">
      {/* kicker */}
      ...
      {INTRO.slice(1).map((p, i) => (
        <p key={i} className="mt-3 text-[13.5px] leading-[1.85] text-[#3f4f4b]">{p}</p>
      ))}
      ...
      <div className="mt-5 rounded-[14px] border border-[#cfe5e2] bg-[linear-gradient(135deg,rgba(23,94,99,0.055),rgba(92,225,230,0.07))] p-5">
        <div className="flex flex-col items-start gap-2.5">
          <span className="inline-flex h-[22px] items-center whitespace-nowrap rounded-full bg-[#175e63] px-2.5 text-[11px] font-bold text-white">참여 혜택</span>
          <span className="text-[14.5px] font-bold text-[#161d1b]">
            유료 부모 검사 <b className="text-[#175e63]">3종 무료 제공</b>
          </span>
        </div>
        <ul className="mt-3 grid gap-2">
          {BENEFIT_LINES.map((l, i) => (
            <li key={i} className="flex gap-2.5 text-[13px] leading-[1.7] text-[#3f4f4b]">
      ...
      <div className="mt-6">
        ...
        >
          <span className="flex items-center gap-2.5">
            ...
            <span className="inline-flex h-5 items-center whitespace-nowrap rounded-full bg-[#e8f3f1] px-2 text-[11px] font-bold text-[#175e63]">3종</span>
          </span>
          <span className="flex shrink-0 items-center gap-2">
            <span className="whitespace-nowrap text-[12px] font-semibold text-[#175e63]">{open ? "접기" : "자세히 보기"}</span>
            <IconChevronDown
              size={16}
        ...
        <div className={`overflow-hidden transition-[max-height] duration-300 ease-out ${open ? "max-h-[640px]" : "max-h-0"}`}>
          <div className="grid gap-2.5 pt-2.5">
            {TESTS.map((t) => (
              <div key={t.code} className="rounded-xl border border-[#e8eded] bg-white p-4 transition-colors hover:border-[#cfe0dc]">
                <div className="flex flex-wrap items-center gap-2.5">
                  <span className="inline-flex h-[22px] items-center whitespace-nowrap rounded-md bg-[#e8f3f1] px-2.5 text-[12px] font-bold text-[#175e63]">{t.code}</span>
                  <span className="text-[13.5px] font-semibold text-[#161d1b]">{t.name}</span>
                </div>
                <p className="mt-2 text-[12.5px] leading-[1.7] text-[#5f6f73]">{t.desc}</p>
              </div>
            ))}
          </div>
        </div>
