...
### 리스트 테이블 정렬 규칙
관리자 리스트 화면(내담자 관리, 검사 관리 등)의 테이블형 UI는 아래 원칙을 따른다.

- 헤더와 데이터 row는 같은 열 정의를 공유해야 한다.
- 헤더 텍스트와 데이터 셀의 정렬 기준은 화면 안에서 일관되게 유지한다.
- 액션 버튼 열은 내용 길이에 따라 흔들리지 않게 고정 폭이나 명확한 grid track을 둔다.
- 짧은 숫자, 날짜, 상태, 액션 열은 좁게 두고, 검사명이나 설명처럼 길어질 수 있는 열에 더 많은 공간을 준다.
- Card 기본 padding이 헤더 정렬을 깨면 해당 리스트 카드에서만 명시적으로 padding을 조정한다.
- 현행 세부 grid 값은 문서가 아니라 실제 컴포넌트(`frontend/src/pages/ClientManagement.tsx`, `frontend/src/pages/TestManagement.tsx`)를 기준으로 확인한다.
