...
## DB 조회 전 확인 규칙
실행 중인 서버의 DB 상태를 확인하는 작업과 로컬 셸에서 직접 DB를 조회하는 작업은 별개로 취급한다.

반드시 먼저 아래를 구분한다.

1. **서버 API로 확인하는가**
   - `http://127.0.0.1:8120`에 떠 있는 FastAPI 프로세스가 이미 어떤 DB에 붙어 있는지 확인한다.
   - 이 경우 우선 `/health`와 실제 API 응답을 기준으로 보고, 셸에서 별도 `APP_ENV=... python ...` 조회를 섞지 않는다.
2. **셸에서 직접 DB를 조회하는가**
   - 명령 실행 전에 `APP_ENV`, 로드될 `env.*` 파일, DB host/port 의미를 확인한다.
   - `APP_ENV` 미지정은 `local.dev`이므로 로컬 SQLite `modular.db`를 본다.
   - `APP_ENV=local.prod`는 RDS 자체가 아니라 로컬 SSH 터널 `127.0.0.1:15432`를 전제로 한다.
   - `APP_ENV=ec2.prod`는 EC2 운영 환경 기준 RDS 직접 접속 설정이다.

금지 패턴:

- `/health`가 `"db":"postgresql"`이라고 해서, 같은 터미널에서 실행하는 Python/SQLAlchemy 조회도 같은 DB를 본다고 가정하지 않는다.
- 운영 데이터 확인 요청에서 `APP_ENV` 없이 Python 조회를 실행하지 않는다.
- `local.prod` 조회 실패를 로컬 DB 데이터 없음으로 해석하지 않는다. 먼저 SSH 터널 `127.0.0.1:15432`가 실제로 열려 있는지 확인한다.
- 실행 서버 DB 확인이 목적일 때, 서버 API로 확인 가능한 내용을 로컬 SQLite 조회로 대체하지 않는다.

운영/실행 서버 데이터 확인의 기본 순서는 아래로 고정한다.

1. `curl http://127.0.0.1:8120/health`로 현재 떠 있는 서버의 DB 종류를 확인한다.
2. 필요한 데이터가 API로 확인 가능하면 해당 서버 API를 사용한다.
3. 직접 SQL 조회가 필요하면 실행 위치와 목적에 맞는 `APP_ENV`를 명시한다.
4. `local.prod`를 쓸 때는 SSH 터널 상태를 먼저 확인한다.
5. 조회 결과를 해석할 때 "서버가 보는 DB"와 "셸 명령이 보는 DB"가 같은지 명시적으로 확인한다.
...
15. `child_test.consent_text` 컬럼 보정
16. `child_test.requires_security_notice` 컬럼 보정
17. `child_test.show_research_notice` 컬럼 보정
18. 관리자 설정 테이블(`admin_settings`) 및 `security_notice_text` 컬럼 보정
19. 개인정보동의 기록 테이블(`client_consent_record`) 보정
20. 검사 실시 임시저장 테이블(`admin_assessment_draft`) 보정
21. `RUN_STARTUP_DATA_MIGRATIONS=1`일 때만 기존 row 재작성 데이터 migration 실행
...
22. `test_profile_config` 테이블 생성 보정
23. 내담자 관계 테이블(`admin_client_relation`) 보정
