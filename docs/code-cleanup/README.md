...
이 디렉터리는 `코드 정리해줘` 요청을 처리할 때 보는 문서 묶음의 지도다.
...
- 시각 규칙, 단계 분해 원칙, 예시 데이터 원칙 같은 상세 기준은 `interactive-flow-spec`에서 본다.
- 제작 절차, 실패 조건, 최종 리뷰 체크리스트는 `playbook`에서 본다.
- HTML/CSS/JS 뼈대는 템플릿 소스에서 직접 본다.
