...
## Browser Routes
- `/report/:submissionId?token={access_token}` — 수검자 결과 열람 (제출 ID + 토큰 query 검증, 사이드바 없음)
- `/admin/report/:submissionId` — 관리자 결과 열람 (`admin_session` 인증, 사이드바 없음)
...
- 상세: [docs/features/scoring-flow.md — 해석문구 조회](scoring-flow.md)
...
## 해석 텍스트 관리
- 해석 텍스트의 운영 데이터 소스는 DB의 `interpret.map` JSON이다.
- `builder.py`는 `custom_test.test_id` 기준으로 `interpret` row를 읽고, 척도/하위척도 코드별 `category` 또는 `raw_score` 범위로 문구를 찾는다.
- 해석이 없으면 빈 문자열을 반환하며, UI는 해석 섹션을 자동으로 숨긴다.

## 검사별 확장 방법
1. 해당 검사 `test_id`의 `interpret.map` JSON에 척도 코드별 해석 문구를 추가한다.
...
## Related Documents
- [docs/features/scoring-flow.md](scoring-flow.md)
- [docs/features/assessment-link-flow.md](assessment-link-flow.md)
