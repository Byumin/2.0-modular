...
## Purpose
관리자 단위로 유지되는 전역 설정을 저장/조회하는 기능을 다룬다. 현재는 개인정보 수집·이용 동의서 기본 본문과 개인정보 보안관리 안내 문구를 저장하는 용도로 사용된다.

## Main Endpoints
- `GET /api/admin/settings/consent`: 현재 관리자 기준 동의서 본문 조회
- `PUT /api/admin/settings/consent`: 동의서 본문 저장(최초면 insert, 기존이면 update)
- `GET /api/admin/settings/security-notice`: 현재 관리자 기준 개인정보 보안관리 안내 문구 조회
- `PUT /api/admin/settings/security-notice`: 개인정보 보안관리 안내 문구 저장

## Main Files
- `app/router/settings_router.py`
- `app/schemas/settings.py`: `ConsentTextIn`, `ConsentTextOut`, `SecurityNoticeTextIn`, `SecurityNoticeTextOut`
...
- 보안관리 안내 문구는 검사별 "개인정보 보안관리 안내" 토글과 조합되어 수검자 프로필 단계의 확인 모달에 노출된다.
- 최초 조회 시 레코드가 없으면 빈 문자열을 반환한다. 저장 시에는 upsert로 동작하며 `updated_at`이 갱신된다.

## Core Flow
1. 관리자가 `/admin/settings`를 열면 동의서 본문과 보안관리 안내 문구를 각각 로드한다.
2. 관리자가 각 textarea에서 본문을 수정하고 저장하면 해당 설정 API로 전송된다.
3. 서비스 레이어가 `AdminSettings` 행 존재 여부를 확인해 insert 또는 update를 수행한다.
4. 이후 검사 실시 링크의 동의/보안관리 확인 단계에서는 이 본문을 기준으로 수검자 확인 UI가 구성된다(자세한 흐름은 `privacy-consent-spec.md` 참고).

## Data Model
`AdminSettings` 주요 필드:
- `admin_user_id`: FK, unique. 관리자 1:1 매핑
- `consent_text`: 동의서 본문 전체 텍스트
- `security_notice_text`: 개인정보 보안관리 안내 문구
- `updated_at`: 수정 시각, 저장할 때마다 갱신

## Notes
- 새 전역 설정이 필요하면 `AdminSettings`에 컬럼을 추가하고 `settings_router.py`에 엔드포인트를 확장한다.
- 검사별 동의/보안관리 사용 여부 토글 자체는 `child_test` 테이블에 보관되며 본 문서 범위가 아니다(개인정보 동의 기능 문서 참고).
