...
## 사전 조건
- EC2에 레포가 `/home/ubuntu/2.0-modular` 경로로 클론되어 있다 (다른 경로면 service 파일을 함께 수정).
- `.venv/`가 생성되어 있고 `requirements.txt`가 설치되어 있다.
- `.env.ec2.prod`이 레포 루트에 작성되어 있다 (RDS 접속 정보 포함). 파일명에 선행 점이 있다 — `EnvironmentFile=` 경로와 일치해야 한다.
- 의존성에 `bcrypt`가 포함되어야 한다 (`pip install -r requirements.txt` 갱신 필요).

## 1. 의존성 갱신 (S-1 bcrypt 적용을 위해)
```bash
cd /home/ubuntu/2.0-modular
git pull
...
```bash
sudo cp /home/ubuntu/2.0-modular/deploy/systemd/insight-backend.service \
