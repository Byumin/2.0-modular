...
현재 주요 browser route인 `/`, `/admin/*`, `/assessment/custom/{token}`, `/report/{token}`, `/admin/report/{submissionId}`는 `frontend/dist/index.html`로 진입한다. `static/`은 운영 React SPA가 아닌 레거시/보고서 정적 자원 용도로만 둔다.
...
- 토큰 기반 공개 접근 (`/report/{token}`) 및 관리자 세션 기반 접근 (`/admin/report/{id}`)
- 척도 계층(종합척도 → 하위척도) 트리 네비게이션
- 상세 내용: [docs/features/report-dashboard.md](/mnt/c/Users/user/workspace/2.0-modular/docs/features/report-dashboard.md)
...
`frontend/dist/index.html`은 `/`, `/admin`, `/admin/*`, `/assessment/custom/{access_token}`, `/report/{token}`, `/admin/report/{submissionId}` route에서 FastAPI가 서빙한다.
